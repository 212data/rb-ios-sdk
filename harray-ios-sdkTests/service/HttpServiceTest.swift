...
class HttpServiceTest: XCTestCase {
    ...
    func test_it_should_call_completion_handler_with_http_downloadable_result_when_response_is_valid() {
        let fakeUrlSession = MockUrlSession(httpDownloadableResult: HttpDownloadableResult(path: URL(string: "http://www.relevantbox.io")!))
        let httpService = HttpService(
            sdkKey: "sdk-key",
            session: fakeUrlSession,
            collectorUrl: "https://c.relevantbox.io",
        ...
        var result: HttpDownloadableResult?
        httpService.downloadContent(endpoint: "https://c.relevantbox.io/img.gif") {
            result = $0
        }
        XCTAssertEqual(URL(string: "http://www.relevantbox.io")!, result?.getPath())
    }
    ...
    func test_it_should_add_sdk_key_to_collector_url() {
        ...
        XCTAssertEqual("https://c.relevantbox.io/sdk-key", httpService.getCollectorUrl())
    }
    ...
    func test_it_should_add_path_to_collector_url() {
        ...
        XCTAssertEqual("https://c.relevantbox.io/path", httpService.getCollectorUrl(path: "path"))
    }
